...
def featureplot(d_train, t_train, n_classes,
                ...
                plt_step = 0.02,
                plt_colors = "rybgcm",
                cnt_colors = ["xkcd:light red","xkcd:pale yellow","xkcd:light blue"]
    ...
    for pairidx, pair in enumerate(pairs):
        ...
        for i, color in zip(range(n_classes), plt_colors):
            ...
            if (splitsample):
                idx = np.where(t_test == c_names[i])
            else:
                idx = np.where(t_train == c_names[i])

            if (c_names is not None):
                #print(pair_test[idx, 0], pair_test[idx, 1])
...
# copy of above featureplot method
# NOTE - quick fix for iris data as not working in featureplot
def irisplot(d_train, t_train, n_classes,
                clf=None,
                d_test=None,
                t_test=None,
                t_names=None,
